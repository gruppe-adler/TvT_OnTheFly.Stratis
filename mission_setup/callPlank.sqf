...
[] spawn {
	while {true} do {
		...
		if (OPFOR_TELEPORTED) exitWith {
			...
			player setVariable ["buildReady", false];
			buildReadyAction1 = player addAction["<t color='#ff5555'>" + localize "str_GRAD_endPlacement" + "</t>",{player setVariable ["buildReady", true]; player removeAction buildReadyAction2;}];
			};
